...
function examples(apiBaseUrl: string, pipeline: string, projectId: string): Record<Language, string> {
  ...
  return {
    curl: [
      ...
      'UPLOAD=$(curl --fail-with-body -sS -X POST "$API_BASE_URL/uploads" -H "X-API-Key: $SLUICE_API_KEY" -H "Idempotency-Key: create-upload-$(date +%s)" -H "Content-Type: application/json" --data "$(jq -n --arg filename "$(basename "$FILE")" --argjson size "$FILE_SIZE" \'{filename:$filename,contentType:"image/png",size:$size,externalSubjectId:"user_123",externalReference:"avatar_2026_08"}\')")',
    ...
    javascript: [
      ...
      "const upload = await api('/uploads', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': randomUUID() }, body: JSON.stringify({ filename: 'photo.png', contentType: 'image/png', size: file.length, externalSubjectId: 'user_123', externalReference: 'avatar_2026_08' }) });",
    ...
    python: [
      ...
      "upload = api('POST', '/uploads', headers={'Idempotency-Key': str(uuid.uuid4())}, json={'filename': 'photo.png', 'contentType': 'image/png', 'size': len(file_bytes), 'externalSubjectId': 'user_123', 'externalReference': 'avatar_2026_08'})",
