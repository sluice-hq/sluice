...
function examples(apiBaseUrl: string, pipeline: string, projectId: string): Record<Language, string> {
  ...
  return {
    curl: [
      ...
      'UPLOAD=$(curl --fail-with-body -sS -X POST "$API_BASE_URL/uploads" -H "X-API-Key: $SLUICE_API_KEY" -H "Content-Type: application/json" --data "$(jq -n --arg filename "$(basename "$FILE")" --argjson size "$FILE_SIZE" \'{filename:$filename,contentType:"image/png",size:$size}\')")',
    ...
    javascript: [
      ...
      "const upload = await api('/uploads', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: 'photo.png', contentType: 'image/png', size: file.length }) });",
    ...
    python: [
      ...
      "upload = api('POST', '/uploads', json={'filename': 'photo.png', 'contentType': 'image/png', 'size': len(file_bytes)})",
