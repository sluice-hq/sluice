    ...
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      <header className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/signup" className={buttonVariants()}>Start building</Link>
        </div>
      </header>
      ...
      <section className="relative mx-auto grid max-w-7xl gap-12 px-5 pb-20 pt-16 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:pb-32 lg:pt-24">
        <div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/signup" className={cn(buttonVariants({ size: 'lg' }), 'h-auto gap-2 px-5 py-3 text-base font-semibold shadow-[0_12px_30px_rgb(35_149_255_/_0.25)]')}>Create a workspace <ArrowRight className="size-4" /></Link>
            <Link href="/app" className={cn(buttonVariants({ variant: 'outline', size: 'lg' }), 'h-auto bg-card px-5 py-3 text-base font-semibold')}>Open dashboard</Link>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 text-center sm:px-8"><BadgeCheck className="mx-auto size-6 text-primary" /><h2 className="mt-4 text-3xl font-semibold tracking-tight">Build media infrastructure that your app can trust.</h2><p className="mx-auto mt-3 max-w-xl text-muted-foreground">Create a project, publish a pipeline, and exercise the developer API from the control plane.</p><Link href="/signup" className={cn(buttonVariants({ size: 'lg' }), 'mt-7 h-auto gap-2 px-5 py-3 text-base font-semibold')}>Get started <Braces className="size-4" /></Link></section>
    </main>
  );
}
